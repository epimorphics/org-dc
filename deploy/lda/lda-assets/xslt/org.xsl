...
<xsl:template match="@href" mode="uri">
	<xsl:choose>
		...
		<xsl:when test="starts-with(., 'http://reference.data.gov.uk/id/day') or    starts-with(., 'http://reference.data.gov.uk/id/year') or    starts-with(., 'http://reference.data.gov.uk/id/gregorian-instant')">
			<xsl:value-of select="."/>
		</xsl:when>
